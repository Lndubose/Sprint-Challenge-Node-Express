...
const checkingPostAction = (req, res, next) => {
  ...
  if (!notes || !description || !project_id) {
    res.status(400).json({
      error:
        "Need a description, notes, and the user that is adding the action."
    });
  } else {
    next();
  }
...
const checkingPutAction = (req, res, next) => {
  ...
  if (!notes || !description) {
    res
      .status(400)
      .json({ error: "Need a description and notes to update action" });
  } else {
    next();
  }
};
...
server.post("/projects", checkingProject, (req, res) => {
  ...
  projectDb
    ...
    .catch(err =>
      res.statusCode(500).json({ error: `Server error --> ${err} ` })
    );
});
...
server.get("/actions/:actionId", (req, res) => {
  const { actionId } = req.params;
  actionDb
    .get(actionId)
    .then(response => res.status(200).json(response))
    .catch(err => res.status(400).json({ error: `No action with that id.` }));
});
